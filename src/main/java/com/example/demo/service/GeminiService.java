...
public class GeminiService {
	...
	private final String GEMINI_API_URL = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=AIzaSyBmejvc1388rc7ry6YHoT6NiUg8K4Vjk38";
	...
	private String buildPrompt(String story, String domain) {
		return switch (domain.toUpperCase()) {
			case "BA" -> context
					+ "You are acting as a Business Analyst. Analyze the following JIRA story and provide your response in the exact format below. Ensure the output is in plain text or HTML (not Markdown). Follow these formatting rules strictly:\r\n"
					+ "\r\n"
			...
			case "QA" -> context
					+ "You are acting as a QA Engineer. Analyze the following JIRA story and provide your response in the exact format below. Ensure the output is in plain text or HTML (not Markdown). Follow these formatting rules strictly:\r\n"
					+ "\r\n"
			...
			case "DEV" -> context
					+ "You are acting as a Developer. Analyze the following JIRA story and provide your response in the exact format below. Ensure the output is in plain text or HTML (not Markdown). Follow these formatting rules strictly:\r\n"
					+ "\r\n"
					+ "- Section headings must be in bold using <b> tags.\r\n"
					+ "- Use bullet points (•) for lists with no extra spacing between bullets.\r\n"
					+ "- Maintain a maximum line spacing of 1 between sections.\r\n"
	...
	public String estimateEffortFromStory(String story) {
		String prompt = context + "You are acting as a Software Estimator."
				...
				// + "keep in mind that a sprint only lasts 10 working days and it contains
				// multiple stories, bugs, tasks etc"
	...
	public String convertRequirementToStory(String requirementText) {
		String prompt = context
				+ "You are acting as a Business Analyst. Convert the following requirement into a well-structured JIRA user story. "
				+ "Your response must follow this format and formatting rules:\r\n"
				+ "** VERY IMPORTANT ** : START ALL THE TEXT IMMEDIATLEY AFTER THE HEADINGS IN THE NEXT LINE  AND GIVE EXACTLY '1' LINE OF SPACE BEWEEN SECTIONS."
				+ "##.START THE POINTS IN THE NEXT LINE. EVERY HEADING SHOULD BE CENETER ALIGNED HAVE THE FONT COLOR #fb4d0a"
				+ "\r\n"
				...
				+ "- Keep a maximum line spacing of 1 between sections.\r\n"
				+ "\r\n"
				+ "You MUST output exactly in the following structure (do not add or remove sections):\r\n"
				+ "\r\n"
				+ "<b>USER STORY</b>\r\n"
	...
	public String generateTestCasesForExcel(String story) {
		String prompt = "You are a QA Engineer. Based on the following JIRA story, generate a list of test cases. " +
				...
				+ "UI/UX scenarios – if relevant, covering usability, responsiveness, and accessibility aspects.\r\n"
				+ "Security scenarios – if relevant, including role-based access, permissions, and data privacy.\r\n"
				+ "Performance scenarios – if applicable, to ensure the feature works well under expected loads."
				+ story;
